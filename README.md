...
**Loopback host:** **`127.0.0.1`** and **`localhost`** are both valid for host-only services the script configures (DeployWerk API/nginx, `DATABASE_URL` host, Mailcow HTTP/HTTPS binds, Garage and Technitium Docker port publishes). Defaults use **`localhost`**; set **`DEPLOYWERK_LOOPBACK_HOST=127.0.0.1`** (or override **`MAILCOW_HTTP_BIND`** / **`MAILCOW_HTTPS_BIND`**) if you want the literal IP everywhere. Traefik **`curl --resolve`** checks default to **`CURL_TRAEFIK_LOOPBACK_IP=127.0.0.1`**. See **`sudo bash scripts/orbytals-install.sh --help`** for the full env list.
