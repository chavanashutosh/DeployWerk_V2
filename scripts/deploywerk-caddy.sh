...
#
# DB, demo seeding, privilege notes: README.md and .env.example. API stderr is appended to
#   ${DEPLOYWERK_API_LOG:-$DEPLOYWERK_STATE_DIR/deploywerk-api.log} (default state dir /var/lib/deploywerk/run).
...
start:  source --env-file, optional DB defaults / --compose-postgres / --prompt-db, start API (logs to
  DEPLOYWERK_API_LOG), wait /api/v1/health, start loopback nginx, same check via HTTP_PORT.
stop / clean / status / run — see README.md.

redeploy:  optional --clean, cargo build API, optional --build-web, then start. Example:
  sudo bash scripts/deploywerk-caddy.sh redeploy --compose-postgres --build-web --http-port 3001 --api-port 8080

Shared flags:
  --api-port --http-port --env-file --web-root --workdir --proto --bind --extra-listen --state-dir
  --prompt-db  --compose-postgres
clean only:  --remove-tmp-nginx
redeploy only:  --clean  --build-web

Env (full list): README.md / .env.example. Common:
  DEPLOYWERK_STATE_DIR  DEPLOYWERK_API_LOG  DEPLOYWERK_API_BIN  DEPLOYWERK_CARGO  DEPLOYWERK_NPM
  DEPLOYWERK_SKIP_PSQL_VERIFY  (set to 1 when DATABASE_URL is sqlite:… — no psql check)  DEPLOYWERK_PSQL_VERIFY_TIMEOUT_SECS  DEPLOYWERK_HEALTH_WAIT_SECS
  DEPLOYWERK_SKIP_HEALTH_WAIT  DEPLOYWERK_PG_*  DEPLOYWERK_POSTGRES_HOST_PORT

Requires: nginx, deploywerk-api binary; redeploy needs cargo; --build-web needs npm.
  sudo PATH:  sudo env "PATH=/root/.cargo/bin:${PATH}" bash scripts/deploywerk-caddy.sh redeploy ...
EOF
}
...
# Rust/sqlx rejects DATABASE_URL when '#' appears in user:password (treated as URL fragment). psql may appear to work.
validate_database_url_no_unencoded_hash() {
  [[ -z "${DATABASE_URL:-}" ]] && return 0
  local rest userinfo
  rest="${DATABASE_URL#*://}"
  userinfo="${rest%%@*}"
  [[ "$userinfo" != *"#"* ]] && return 0
  die "DATABASE_URL must not contain '#' in the user/password (use %23 or change password). Unencoded '#' breaks deploywerk-api (sqlx)."
}
...
health_wait_failed_help() {
  ...
  echo "  - Verify DB:  psql \"\$DATABASE_URL\" -c 'select 1'   and API log: $(api_log_file)" >&2
  echo "  - Manual check:  curl -sS \"${url}\"   RUST_LOG=debug for more detail." >&2
  tail_api_log_hint
}
...
api_log_file() {
  state_paths
  echo "${DEPLOYWERK_API_LOG:-${STATE_DIR}/deploywerk-api.log}"
}
...
tail_api_log_hint() {
  ...
  [[ -f "$f" ]] || return 0
  echo "" >&2
  echo "Last 30 lines of $f:" >&2
  tail -n 30 "$f" 2>/dev/null >&2 || true
}
...
cmd_start() {
  ...
  touch "$api_log" || die "cannot write API log: $api_log"

  echo "Starting deploywerk-api: $api_bin HOST=$HOST PORT=$PORT workdir=$workdir log=$api_log"
  ( cd "$workdir" && exec >>"$api_log" 2>&1 "$api_bin" ) &
  local apid=$!
...
cmd_status() {
  ...
  if [[ -f "$API_PID_FILE" ]]; then
    ...
    if kill -0 "$apid" 2>/dev/null; then
      ...
      if command -v curl >/dev/null 2>&1; then
        if curl -sfS "http://127.0.0.1:${API_PORT}/api/v1/health" -o /dev/null; then
          echo "deploywerk-api health: OK http://127.0.0.1:${API_PORT}/api/v1/health"
        else
          echo "deploywerk-api health: failed (curl http://127.0.0.1:${API_PORT}/api/v1/health)" >&2
        fi
      fi
    else
...
# mode=run: start|stop|clean|restart|status|run. mode=redeploy: redeploy-only --clean/--build-web.
parse_common_flags() {
  local mode="${1:-run}"
  shift || true
  case "$mode" in
    ...
    redeploy)
      ...
      ;;
    *)
      die "internal: parse_common_flags mode"
      ;;
  esac
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --remove-tmp-nginx)
        [[ "$mode" == "run" ]] || die "--remove-tmp-nginx is only valid for clean (and commands that share run flags)"
        CLEAN_REMOVE_TMP_NGINX=1
        shift
        ;;
      --clean)
        [[ "$mode" == "redeploy" ]] || die "--clean is only valid for redeploy"
        REDEPLOY_CLEAN=1
        shift
        ;;
      --build-web)
        [[ "$mode" == "redeploy" ]] || die "--build-web is only valid for redeploy"
        REDEPLOY_BUILD_WEB=1
        shift
        ;;
      -h|--help) usage; exit 0 ;;
      *) die "unknown flag: $1" ;;
...
main() {
  ...
  case "$sub" in
    help|-h|--help) usage ;;
    start) parse_common_flags run "$@"; cmd_start ;;
    stop) parse_common_flags run "$@"; cmd_stop ;;
    clean) parse_common_flags run "$@"; cmd_clean ;;
    restart) parse_common_flags run "$@"; cmd_stop; cmd_start ;;
    redeploy) parse_common_flags redeploy "$@"; cmd_redeploy ;;
    status) parse_common_flags run "$@"; cmd_status ;;
    run) parse_common_flags run "$@"; cmd_run ;;
